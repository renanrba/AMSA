...
**Status:** Rodada 7 — corrige a métrica "tempo médio de resposta": `wa.me` não dá visibilidade de conversa a terceiros, então ela não é mensurável na Fase 1; substituída por proxies reais (RF-49 tempo até confirmação, RF-50 tag autorreportada "responde rápido"), com relay via WhatsApp Business Platform (Cloud API) registrado como opção de Fase 3 se o time quiser o dado real. Rodada 6: estratégias de validação de reviews sem confirmação de pagamento (confirmação não bloqueante do prestador, badge "Verificado", evidência opcional, contador de trabalhos concluídos, sinalização automática para moderação, denúncia estruturada e badge de KYC). Rodada 5: catálogo de categorias reestruturado em 12 grupos/48 subcategorias orientadas à operação de temporada (RF-09). Rodada 4: módulo renomeado para **Conecta Prestadores** (era "Comunidade de Prestadores"); métricas no Admin, upload de documentos no cadastro, disponibilidade/horários como filtro, cobertura em cascata (estado → cidade → bairros)
**Data:** 2026-08-29
