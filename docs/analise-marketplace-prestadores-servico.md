...
**Status:** Rodada 12 — início da implementação real. Migrations SQL completas escritas em `hospedeai-v2/supabase/migrations/` (seção 10, itens 14-15), prontas pra rodar manualmente no Supabase; RLS incluída (`is_provider_owner()` + policies); bairros de João Pessoa semeados por nome (fonte oficial), coordenadas via script de geocodificação real (Google Maps Geocoding API, chave a cargo do time); Products/Prices do Stripe ficam a cargo do time (`stripe_price_id_mensal`/`_anual` na tabela, preencher depois de criados). RF-36 esclarecido: "Gerenciar Planos" tem só o padrão visual reaproveitado — a tela atual (`PlansManagement.tsx`) é só `localStorage`, os planos do prestador (`provider_subscription_plan`) ficam num schema real. WhatsApp Business API confirmado só como item de roadmap (Fase 3), sem mudança de escopo. Rodada 11: landing page pública dupla audiência (RF-54). Rodada 10: preços de referência no cadastro (RF-03). Rodada 9: card em `/selection`. Rodada 8: "Buscar prestador" na aba Manutenção. Rodada 7: corrige "tempo médio de resposta". Rodada 6: validação de reviews. Rodada 5: catálogo em 12 grupos/48 subcategorias. Rodada 4: módulo renomeado para **Conecta Prestadores** (era "Comunidade de Prestadores")
**Data:** 2026-08-29
...
- **RF-36** *(esclarecido — rodada 12)* Gestão de categorias, planos e preços de assinatura do prestador. **Reaproveita o padrão visual** da tela "Gerenciar Planos" já existente (`components/admin/PlansManagement.tsx`) — nome PT/EN, preço BRL, Stripe Price ID mensal/anual, toggles de feature — mas **não a persistência**: aquela tela hoje só edita `services/plans.ts` em memória e grava override em `localStorage` (`platform_plans_config`), sem tocar o banco. Os planos do prestador ficam numa tabela real (`provider_subscription_plan`, seção 8), editável via uma aba nova (ou tela irmã) dentro de "Gerenciar Planos" que lê/grava direto no Supabase.
