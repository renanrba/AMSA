...
**Status:** Rodada 8 — corrige o ponto de entrada do módulo com base em código real: entrada contextual (RF-12) sai do botão "Preciso de um profissional" genérico e vira o botão **"Buscar prestador"** dentro da aba **Manutenção** já existente em produção (reaproveitando os campos `responsible_name`/`responsible_phone` de `maintenance_events`); entrada geral (RF-13) vai para dentro do grupo de sidebar **"Rotina"** já existente, não um grupo novo. Rodada 7: corrige a métrica "tempo médio de resposta" (`wa.me` não dá visibilidade de conversa a terceiros), substituída por proxies reais (RF-49/RF-50), com relay via WhatsApp Business Platform registrado como opção de Fase 3. Rodada 6: estratégias de validação de reviews sem confirmação de pagamento. Rodada 5: catálogo de categorias reestruturado em 12 grupos/48 subcategorias. Rodada 4: módulo renomeado para **Conecta Prestadores** (era "Comunidade de Prestadores"); métricas no Admin, upload de documentos no cadastro, disponibilidade/horários como filtro, cobertura em cascata (estado → cidade → bairros)
**Data:** 2026-08-29
...
- **RF-13** Ponto de entrada geral: item **"Conecta Prestadores"** dentro do grupo de menu **"Rotina"** já existente (junto de Estoque, Manutenção, Previsão Financeira, Área Host, Avaliações) — **não** um grupo novo de sidebar. O sidebar real do gestor (`MainDashboard.tsx`) tem 3 grupos — Operação / Rotina / Controle — e Conecta Prestadores se encaixa em Rotina por ser também uma ferramenta operacional recorrente por imóvel. Usado para busca livre por categoria + localidade, sem partir de um evento de manutenção específico (ex.: categorias de hospitality/administrativo, rodada 5, que não nascem de um evento de manutenção).
