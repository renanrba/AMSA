...
**Status:** Rodada 13 — as 6 migrations da rodada 12 já rodaram em produção no Supabase (confirmado) e `GOOGLE_MAPS_SERVER_API_KEY` já está configurada nas environments do Vercel. RF-36 passa de "esclarecido" para **implementado**: `components/admin/PlansManagement.tsx` ganhou uma segunda aba, "Planos do Prestador (Conecta Prestadores)" (`components/admin/ProviderPlansManagement.tsx`, novo), que lê/grava de verdade em `provider_subscription_plan` via Supabase — a aba "Planos do Gestor" original continua intacta (`services/plans.ts` + `localStorage`, sem mudança). Também entregue: `scripts/backfill-properties-geocoding.mjs`, job de backfill que geocodifica `properties.address` (texto livre) via Geocoding API real, preenchendo `lat`/`lng`/`city`/`neighborhood` dos imóveis já cadastrados — cidade/bairro extraídos de `address_components` da resposta, nunca inventados; endereços não localizados (ou resolvidos só aproximadamente) saem num relatório de revisão manual. Detalhes em seção 10, item 16. Ainda pendente: rodar `geocode-bairros-joao-pessoa.mjs` (a chave já existe, só falta rodar) e criar Products/Prices no Stripe. Rodada 12: migrations SQL completas escritas em `hospedeai-v2/supabase/migrations/` (seção 10, itens 14-15), RLS incluída (`is_provider_owner()` + policies + triggers de guarda de coluna); bairros de João Pessoa semeados por nome (fonte oficial). Rodada 11: landing page pública dupla audiência (RF-54). Rodada 10: preços de referência no cadastro (RF-03). Rodada 9: card em `/selection`. Rodada 8: "Buscar prestador" na aba Manutenção. Rodada 7: corrige "tempo médio de resposta". Rodada 6: validação de reviews. Rodada 5: catálogo em 12 grupos/48 subcategorias. Rodada 4: módulo renomeado para **Conecta Prestadores** (era "Comunidade de Prestadores")
**Data:** 2026-08-29
...
- **RF-36** *(implementado — rodada 13)* Gestão de categorias, planos e preços de assinatura do prestador. "Gerenciar Planos" (`components/admin/PlansManagement.tsx`) ganhou uma aba dedicada, "Planos do Prestador (Conecta Prestadores)" (`components/admin/ProviderPlansManagement.tsx`), que **reaproveita o padrão visual** da aba original (nome PT/EN, preço BRL, Stripe Price ID mensal/anual, toggles de feature) mas com **persistência real**: lê/grava direto em `provider_subscription_plan` (seção 8) via Supabase, com RLS já concedendo acesso total ao Admin (`is_current_user_platform_admin()`). A aba "Planos do Gestor" original continua como estava — `services/plans.ts` + `localStorage`, sem mudança.
...
- **provider_subscription_plan** *(nome final — rodada 12; era "subscription_plan" nas rodadas anteriores)*: id, slug (`autonomo`/`equipe`/`empresa`), nome_pt, nome_en, preco_mensal_brl, preco_anual_brl, stripe_price_id_mensal, stripe_price_id_anual, member_limit (nulo = ilimitado), selo, tem_destaque_busca, nivel_relatorios, suporte_prioritario, ativo — editável pelo Admin (RF-36), ver proposta inicial de valores na seção 5.6.1. Migration real já escrita, ver seção 10 item 14.
...
14. **Migrations reais escritas — rodada 12.** Todo o schema da seção 8 virou SQL de verdade em `hospedeai-v2/supabase/migrations/`, seguindo a convenção real do projeto (timestamp `YYYYMMDDHHMMSS_nome.sql`, `uuid_generate_v4()`, RLS habilitado por tabela): `20260830100000_marketplace_catalog.sql` (categorias + estados/cidades/bairros, com os 12 grupos/48 subcategorias e os bairros de João Pessoa já semeados por nome, fonte oficial da Prefeitura), `20260830100100_marketplace_providers.sql` (cadastro do prestador), `20260830100200_marketplace_plans_billing.sql` (`provider_subscription_plan` já semeada com a proposta da seção 5.6.1), `20260830100300_marketplace_engagement.sql` (contato/review/denúncia, com trigger `sync_review_verified` mantendo `review.verified` em sincronia com `service_contact.confirmed_by_provider` — RF-45), `20260830100400_marketplace_existing_table_additions.sql` (aditiva em `properties`/`maintenance_events`), `20260830100500_marketplace_rls.sql` (`is_provider_owner()` + todas as policies). Entregues como arquivo pra rodar manualmente no SQL Editor do Supabase (não aplicadas automaticamente) — README de execução em `supabase/migrations/20260830_CONECTA_PRESTADORES_LEIA-ME.md`.
15. **Geocodificação real dos bairros — rodada 12.** Os nomes dos ~44 bairros de João Pessoa semeados na migration acima vêm de fonte pública oficial (mapa da Prefeitura); `lat`/`lng` ficam `NULL` de propósito — nenhuma coordenada foi inventada de memória. `hospedeai-v2/scripts/geocode-bairros-joao-pessoa.mjs` chama a Geocoding API real do Google Maps (variável `GOOGLE_MAPS_SERVER_API_KEY`, chave server-side — diferente da `VITE_GOOGLE_MAPS_API_KEY` já usada no front-end) pra preencher essas colunas; roda com `--dry-run` primeiro pra conferência. Precisa de uma chave real, que a análise não tem acesso — script fica pronto pra rodar com a chave de vocês.
16. **RF-36 implementado e backfill de `properties` — rodada 13.** Confirmado que as 6 migrations já rodaram em produção e a chave `GOOGLE_MAPS_SERVER_API_KEY` já está nas environments do Vercel. `components/admin/ProviderPlansManagement.tsx` (novo) implementa RF-36 de verdade: `tsc --noEmit` passa limpo, CRUD (só update — os 3 planos já vêm semeados por `slug`, sem UI de criar/apagar) via cliente Supabase do browser, mesmo padrão de escrita direta já usado em `TrialsTab.tsx` (`handleDelete`/`handleToggleStatus`), amparado pela policy `"Platform admin can manage plans"` (RLS, migration 6). Também entregue `hospedeai-v2/scripts/backfill-properties-geocoding.mjs`: geocodifica `properties.address` (campo de texto livre, sem CEP nem cidade/bairro estruturados — `20260701000400_properties.sql`) via Geocoding API real, preenchendo `lat`/`lng`/`city`/`neighborhood`; cidade e bairro vêm de `address_components` da própria resposta da API (tipos `locality`/`administrative_area_level_2` e `sublocality_level_1`/`sublocality`/`neighborhood`), nunca parseados do texto livre nem inventados. Suporta `--dry-run` e `--limit=N` (rodar em lotes pra orçar custo antes do backfill completo); endereços não localizados, ou resolvidos só de forma aproximada (endereço incompleto), saem num relatório final pra revisão manual — rodar de novo após corrigir o cadastro.
