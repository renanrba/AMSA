...
**Status:** Rodada 9 — Conecta Prestadores vira também um card independente na tela real `/selection` (`DashboardSelection.tsx`), desbloqueado em **todos os planos, incluindo Starter/free**, acessível mesmo por gestor sem nenhum imóvel cadastrado (RF-52/RF-53) — decisão deliberada de crescimento dos dois lados do marketplace (mais gestores gratuitos → mais demanda percebida → mais motivo pro prestador pagar pra ficar visível). Gestor sem imóvel usa o seletor de localização em cascata (RF-04) como fluxo principal de busca, não como fallback. Rodada 8: corrige o ponto de entrada contextual (RF-12) com base em código real — vira o botão "Buscar prestador" dentro da aba Manutenção já existente. Rodada 7: corrige "tempo médio de resposta" (não mensurável via `wa.me`). Rodada 6: validação de reviews sem confirmação de pagamento. Rodada 5: catálogo em 12 grupos/48 subcategorias. Rodada 4: módulo renomeado para **Conecta Prestadores** (era "Comunidade de Prestadores")
**Data:** 2026-08-29
